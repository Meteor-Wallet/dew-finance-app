...
const SimulateModal = () => {
  ...
  const isSimulateModalOpen = useVaultActionStore((s) => s.isSimulateModalOpen);
  ...
  useEffect(() => {
    if (isSimulateModalOpen) {
      ...
      const interval = setInterval(() => {
        step++;
        setCurrentStep(step);
        if (step === SIMULATE_ITEMS.length) clearInterval(interval);
      }, 2000);
    ...
    >
      <div className="w-full md:w-[600px] p-6 border-t border-t-modal-border md:border md:border-modal-border rounded-t-2xl md:rounded-2xl">
        <h2 className="text-2xl font-semibold mb-0 mt-4">
          Simulate Strategy Flow
        </h2>
        <p className="text-sm text-gray font-regular mb-4">
          Run a test of the vault's investment strategy to preview expected
          returns, risks, and fund movements without committing real assets.
        </p>

        <div className="dot-grid w-full h-[60vh] overflow-y-auto no-scrollbar p-8 flex flex-col gap-8 items-center">
          {SIMULATE_ITEMS.slice(0, currentStep).map((item, index) => (
            <div
              key={index}
              ref={(el) => {
              ...
              >
                <div className="flex justify-between mb-2 text-xs text-gray mb-3">
                  <div className="flex gap-1.5 items-center">
                    <img src={nearLogo} className="w-5" />
                    {item.title}
                  </div>
